...
float Interpolation::get_point(float *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y) {
  if (x < 0)        x = 0;
  if (y < 0)        y = 0;
  if (x >= cols)    x = cols - 1;
  if (y >= rows)    y = rows - 1;
...
void Interpolation::set_point(float *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y, float f) {
...
void Interpolation::interpolate_image(float *src, uint8_t src_rows, uint8_t src_cols, float *dest, uint8_t dest_rows, uint8_t dest_cols) {
...
float Interpolation::cubicInterpolate(float p[], float x) {
...
float Interpolation::bicubicInterpolate(float p[], float x, float y) {
    float arr[4] = {0,0,0,0};
...
void Interpolation::get_adjacents_1d(float *src, float *dest, uint8_t rows, uint8_t cols, int8_t x, int8_t y) {
...
void Interpolation::get_adjacents_2d(float *src, float *dest, uint8_t rows, uint8_t cols, int8_t x, int8_t y) {
    //Serial.print("("); Serial.print(x); Serial.print(", "); Serial.print(y); Serial.println(")");
